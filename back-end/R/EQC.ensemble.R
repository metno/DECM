...
## 'observations' (one or several reanalyses). The testing is done through rank-statistics, and we expect
## that the rank number follows a uniform distribution if the simulated results and observations belong to
...
EQC.ensemble <- function(obs=c('air.mon.mean.nc','ETAINT_t2m.mon.nc','MERRA'),
  ...
  y1 <- esd::subset.field(esd::retrieve.default(obs[1]),it=it,is=is)
