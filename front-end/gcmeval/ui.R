...
synch <-
  match(paste(meta$gcm[im2], meta$gcm_rip[im2]),
        paste(meta$gcm[im], meta$gcm_rip[im]))
gcmnames <-
  paste(seq(length(synch)), ": ", meta$gcm[synch], ".", meta$gcm_rip[synch], sep =
          "")
...
dashboardPage(
  ...
  dashboardHeader(
    ...
    tags$li(
      a(
        ...
        img(
          src = 'https://climatedatasite.net/wp-content/uploads/2018/02/banner_c3s.png',
          title = "DECM website",
          height = "40px"
        ),
  ...
  dashboardSidebar(
    ...
    column(12, actionButton(
      "randomize", label = "Random", width = '110px'
    )),
    column(12, actionButton(
    ...
    column(
      12,
      checkboxInput("gcm.repeat", "Only one simulation from each GCM", value =
                      FALSE)
    ),
    column(
      12,
    ...
    fluidRow(
      column(
        ...
        box(
          ...
          collapsed = FALSE,
          width = '100%',
          tags$img(src = "banner_c3s.png", width =
                     "100%")
        )
      ),
      column(
