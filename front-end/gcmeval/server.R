...
shinyServer(function(input, output) {
  ...
  output$MetricText  <- renderText({
    ...
    {
      print("Number of GCMs with pr not equal to numbers of GCMS with tas! ABORT")
      exit
    }
    ...
    tasRanks <- array(NA,c(length(gcmst),5,4,length(Regions)))
    prRanks <- array(NA,c(length(gcmsp),5,4,length(Regions)))
    ...
    for (si in 1:5)
    {
      for (ri in 1:length(Regions))
      {
        ...
        #bias
        tasRanks[,si,1,ri] <- rank(abs(sapply(gcmst, function(gcm) mean(sapply(Seasons[[si]], function(s)
          stats$tas$present[[gcm]][[region]][["mean"]][[s]]-stats$tas$present[[tasref]][[region]][["mean"]][[s]])))))
        prRanks[,si,1,ri] <- rank(abs(sapply(gcmsp, function(gcm) mean(sapply(Seasons[[si]], function(s)
          stats$pr$present[[gcm]][[region]][["mean"]][[s]]*60*60*24-stats$pr$present[[prref]][[region]][["mean"]][[s]]*1E3)))))
        
        #sd.ration
        tasRanks[,si,2,ri] <- rank(abs(1-sapply(gcmst, function(gcm) mean(sapply(Seasons[[si]], function(s)
          stats$tas$present[[gcm]][[region]][["spatial.sd"]][[s]]/stats$tas$present[[tasref]][[region]][["spatial.sd"]][[s]])))))
        prRanks[,si,2,ri] <- rank(abs(1-sapply(gcmsp, function(gcm) mean(sapply(Seasons[[si]], function(s)
          stats$pr$present[[gcm]][[region]][["spatial.sd"]][[s]]*60*60*24/stats$pr$present[[prref]][[region]][["spatial.sd"]][[s]]/1E3)))))
        
        #corr
        tasRanks[,si,3,ri] <- rank(1-sapply(gcmst, function(gcm) mean(sapply(Seasons[[si]], function(s)
          stats$tas$present[[gcm]][[region]][["corr"]][[s]]))))
        prRanks[,si,3,ri] <- rank(1-sapply(gcmsp, function(gcm) mean(sapply(Seasons[[si]], function(s)
          stats$pr$present[[gcm]][[region]][["corr"]][[s]]))))
        
        #CMPI (combined model performance index)
        tasRanks[,si,4,ri] <- rank(-1*sapply(gcmst, function(gcm) mean(sapply(Seasons[[si]], function(s)
          stats$tas$present[[gcm]][[region]][["e"]]))))
        prRanks[,si,4,ri] <- rank(-1*sapply(gcmsp, function(gcm) mean(sapply(Seasons[[si]], function(s)
    ...
    paste("The mean weighted model performance rank of the selected models is ","<font size = +1, font color=\"",legcolsrank[meanRelMetricsIndx],"\"><b> Rank", round(mean_weightedrank), "of",length(gcmst),"</b></font>.
